...
class Boss {
    constructor(level = 1) {
        ...
        // 스테이지 6+ 보스2: 더 많은 체력
        const isBoss2 = currentStage >= 6;
        const baseHp = 2500 * level + (level === 10 ? 15000 : 0);
        this.hp = isBoss2 ? Math.floor(baseHp * 1.4) : baseHp; // 보스2는 40% 더 많은 체력
        this.maxHp = this.hp;
        ...
        this.speedY = (isBoss2 ? baseSpeed * 1.3 : baseSpeed) * bossMoveMultiplier * gameSpeed; 
    ...
    update(timestamp) {
        ...
        const interval = isBoss2 ? baseInterval * 0.7 : baseInterval; // 보스2는 30% 더 빠른 공격
        
        if (timestamp - this.lastAttackTime > interval * bossAttackIntervalMultiplier) {
            const p = Math.random();
            // 모바일에서는 7스테이지 이후부터만 소환 패턴 사용 (초보 보호)
            // 보스2는 항상 소환 가능
            const canSummonMinions = isBoss2 || !isMobileEasyModeActive() || currentStage >= 7;
            
            // 디버깅용: 모바일에서 소환 가능 여부 로그
            if (isMobileEasyModeActive() && currentStage < 7 && !isBoss2) {
                // 모바일 7스테이지 미만: 소환 금지
            }

            if (p < 0.35 && canSummonMinions) {
                // 부하 소환 패턴
                const summonCount = isBoss2 ? 5 : 3; // 보스2는 5마리 소환
                for (let i = 0; i < summonCount; i++) {
                    const e = new Enemy(this.x, this.y + this.height / 2 + (i - Math.floor(summonCount/2)) * 60);
                    ...
                    // 보스2 소환몹: 더 빠르고 튼튼함
                    e.speed = (isBoss2 ? 7 : 5) * summonEnemySpeedMultiplier; 
                    e.hp = Math.round((isBoss2 ? 350 : 200) * summonEnemyHpMultiplier); 
                    enemies.push(e); // EASY MODE
                }
            } else if (p < 0.6) {
                // 보스 에너지 볼 - 보스2는 더 크고 빠름
                const bossProjSpeed = isMobileEasyModeActive() ? (isBoss2 ? 9 : 6) * EASY_MODE_CONFIG.bossProjectileSpeedMultiplier : (isBoss2 ? 9 : 6);
                const bossProjDamage = isMobileEasyModeActive() ? (isBoss2 ? 3 : 2) : (isBoss2 ? 4 : 3);
                const b = new Projectile(this.x, this.y + this.height / 2, -bossProjSpeed, 0, 0);
                b.isBossEnergyBall = true; 
                b.lifeDamage = bossProjDamage; 
                b.width = isBoss2 ? 60 : 45; 
                b.height = isBoss2 ? 60 : 45; 
                projectiles.push(b);
            } else if (p < 0.8) {
                // 보스 산탄 - 보스2는 5방향 산탄
                const bossScatterSpeed = isMobileEasyModeActive() ? (isBoss2 ? 11 : 8) * EASY_MODE_CONFIG.bossProjectileSpeedMultiplier : (isBoss2 ? 11 : 8);
                const scatterCount = isBoss2 ? 5 : 3; // 보스2는 5방향
                const angleStep = isBoss2 ? 0.8 : 1.5;
                ...
                for (let i = 0; i < scatterCount; i++) {
                    ...
                    if (isBoss2) b.lifeDamage = 2; // 보스2 산탄도 데미지 증가
                    projectiles.push(b);
                }
            } else if (isBoss2) {
                // 보스2 전용: 연속 에너지 볼 (3연발)
    ...
    draw(ctx) {
        // 스테이지 6+에서는 보스2 이미지 사용
        const isBoss2 = currentStage >= 6;
        const img = ImageLoader.get(isBoss2 ? 'boss2' : 'boss');
        if (img) {
            // 보스2는 전체 이미지를 사용 (스프라이트 맵 사용 안함)
            if (isBoss2) {
                const boss2Scale = 0.4; // 보스2 이미지 크기 조정
                const drawW = img.width * boss2Scale;
                const drawH = img.height * boss2Scale;
        ...
        const bossName = currentStage >= 6 ? 'BOSS: BLUE RABBIT EVOLVED' : 'BOSS: BLUE RABBIT';
