...
const STAGE_DATA = {
    ...
    10: { goal: 150, enemySpeed: 5.0, spawnRate: 800, homing: true, homingSpeed: 2.0, enemyShoot: true, sky: '#000000', city: '#000000', fog: '#450a0a', storyText: "■■■■ STAGE 10: THE LAST CROWN ■■■■\n\n마침내 도달한 50층, 방송국 최상층 스카이라운지는 더 이상 도시를 조망하는 화려한 공간이 아니었다. 그곳은 거대한 살점과 푸른 털이 뒤엉켜 맥동하는 '청토끼의 심장부'였다.\n\n중앙에는 프로젝트의 주동자였던 미친 과학자가 괴물들의 왕과 하나로 융합된 채 연이를 기다리고 있었다. 놈의 등 뒤로는 수만 마리의 청토끼들을 조종하는 신경삭들이 타워 전체로 뻗어 나가고 있었다.\n\n전투의 서막: 마지막 탄환\n연이의 손에 남은 것은 반쯤 부서진 산탄총과 단 한 탄창의 권총뿐. 어깨의 상처에서는 끊임없이 피가 흘러나와 분홍빛 털을 검붉게 물들였다. 하지만 그녀의 눈동자에는 공포 대신, 멸족당한 일족의 슬픔과 세상을 지옥으로 만든 자를 향한 차가운 살의만이 서려 있었다.\n\n'네놈이 만든 진화의 끝이 고작 이 구역질 나는 고기 덩어리냐? 내 일족의 영혼을 담아, 그 왕관을 부숴주마.'\n\n결전의 양상\n하늘은 칠흑 같은 어둠 속에 핏빛 번개가 내리쳤다. '청토끼의 왕'은 비정상적으로 발달한 뒷다리로 지면을 박차며 연이의 시야를 순식간에 좁혀왔다. 놈이 포효할 때마다 사방의 고치에서 변이체들이 쏟아져 나왔고, 놈의 등에서 돋아난 촉수들은 소총탄을 쏘아대며 연이를 압박했다.\n\n연이는 바닥을 구르고, 부서진 콘크리트 기둥을 방패 삼아 도약했다. 공중에서 쏜 산탄총의 불꽃이 놈의 갑각을 때렸지만, 왕은 고통조차 잊은 듯 달려들었다. 연이는 자신의 생명력을 태우듯 아드레날린을 쥐어짜며 적의 심장, 즉 과학자의 본체가 숨겨진 가슴 중앙을 조준했다.\n\n최후의 일격\n전투가 정점에 달했을 때, 연이는 놈의 거대한 발톱에 옆구리를 꿰뚫렸다. 비명이 터져 나올 법한 고통이었지만, 그녀는 오히려 놈의 몸속으로 더 깊숙이 파고들었다. 그녀는 품 안에서 핀을 뽑은 마지막 수류탄을 놈의 벌어진 아가리 속으로 밀어 넣었다.\n\n'이건 내 가족의 몫이다. 그리고 이건... 살아남은 자들의 몫이야!'\n\n굉음과 함께 타워 최상층이 폭발하며 불꽃에 휩싸였다. 거대한 왕의 신체가 무너져 내리고, 도시 전체를 지배하던 하이브 마인드의 비명 소리가 네온 시티의 밤공기를 찢으며 사그라들었다." }
};
...
class Enemy {
    constructor(x, y) {
        ...
        this.width = isStage6Plus ? 85 : 64;
        this.height = isStage6Plus ? 85 : 64;
        // 적 체력 대폭 상향 및 난이도 증가
    ...
    draw(ctx) {
        ...
        if (currentStage >= 8) {
            enemyAssetKey = 'enemy3'; // 스테이지 8+: 청토끼 3
        } else if (currentStage >= 4) {
            enemyAssetKey = 'enemy2'; // 스테이지 4-7: 청토끼 2
        }
...
function drawWinScreen() {
    ...
    if (winImg) {
        ctx.save();
        const winSize = Math.min(canvas.width * 0.3, 200);
        ctx.shadowBlur = 40; ctx.shadowColor = '#f1c40f';
        ctx.drawImage(winImg, canvas.width / 2 - winSize / 2, 30, winSize, winSize);
        ctx.restore();
    }
    ...
    ctx.fillText('REQUIEM FOR THE PINK DAWN', canvas.width / 2, 280);
    ctx.restore();
    ...
    // 텍스트 클리핑 영역 설정
    ctx.beginPath();
    ctx.rect(20, textStartY - 10, canvas.width - 40, canvas.height - textStartY - 80);
    ctx.clip();
    
    // 에필로그 텍스트 그리기 (자동 스크롤)
    const lines = EPILOGUE_TEXT.split('\n');
    const totalHeight = lines.length * lineHeight;
    const scrollOffset = (winScreenAnimationTime * 0.5) % (totalHeight + canvas.height);
    
    lines.forEach((line, index) => {
        const y = textStartY + index * lineHeight - scrollOffset;
        
        // 화면 내에 보이는 텍스트만 그리기
        if (y > textStartY - 20 && y < canvas.height - 60) {
            // 섹션 제목은 노란색으로
    ...
    ctx.font = '8px "Press Start 2P"';
    ctx.fillText('클릭하여 메인 화면으로', canvas.width / 2, canvas.height - 30);
    ctx.restore();
}
